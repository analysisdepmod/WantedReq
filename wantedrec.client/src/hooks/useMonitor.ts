...
export function useMonitor(intervalSec = 3) {
    ...
    const [feedStates,    setFeedStates]    = useState<Record<number, CameraFeedState>>({});
    ...
    const localCameras = useMemo(
        () => cameras.filter(c => detectCameraKind(c) === 'local')
                     .sort((a, b) => a.cameraId - b.cameraId),
        [cameras],
    );

    // ── مطابقة device لكل كاميرا محلية ───────────────────
    const getDeviceId = useCallback((cam: CameraDto): string | null => {
    ...
    const updateFeed = useCallback((cameraId: number, patch: Partial<CameraFeedState>) => {
        setFeedStates(prev => ({
            ...prev,
            [cameraId]: { cameraId, result: null, pending: false, frames: 0, ...prev[cameraId], ...patch },
        }));
    }, []);
    ...
    const onRecognized = useCallback((
        cam: CameraDto,
        result: LiveRecognitionResultDto,
    ) => {
        ...
        if (newEvents.length > 0) {
            setEvents(prev => [...newEvents, ...prev].slice(0, 100)); // max 100
